...
function initMap() {
    ...
    autocomplete(document.getElementById("locationInput"), legend, markers);
}
...
function autocomplete(inp, arr, markers) {
    ...
    inp.addEventListener("input", function(e) {
        ...
        for (i = 0; i < arr.length; i++) {
          ...
          if (arr[i].location.substr(0, val.length).toUpperCase() == val.toUpperCase()) {
            /*create a DIV element for each matching element:*/

            if(prevEntry == arr[i].location){
                //skip
                break;
            }

            prevEntry = arr[i].location

            b = document.createElement("DIV");
            /*make the matching letters bold:*/
            b.innerHTML = "<strong>" + arr[i].location.substr(0, val.length) + "</strong>";
            b.innerHTML += arr[i].location.substr(val.length);
            /*insert a input field that will hold the current array item's value:*/
            b.innerHTML += "<input type='hidden' value='" + arr[i].location + "'>";
